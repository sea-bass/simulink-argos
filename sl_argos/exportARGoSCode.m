...
% Replace all the placeholder tokens in the header file with the model/controller name
...
% Replace all the placeholder tokens in the source file with the model/controller name
...
% Add all source (.cpp) files, except the autogenerated main
srcList = '';
numSrc = numel(buildInfo.Src.Paths);
for idx = 1:numSrc
    srcFileName = buildInfo.Src.Files(idx).FileName;
    if ~strcmp(srcFileName,'ert_main.cpp')
        srcList = [srcList newline '            ' fullfile(modelName,srcFileName)]; %#ok<AGROW>
    end
end

% Replace all the placeholder tokens with the model/controller name and source lists
